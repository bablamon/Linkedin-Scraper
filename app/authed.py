...
class AuthedProfileFetcher:
    ...
    def __init__(self, *, pacer, proxy_pool, session_store, settings, transport=None):
        ...
        # Reuse the guest transport: it warms the cookie jar with a root GET
        # before the real request, which is what establishes `lidc` cleanly.
        # Without that warm-up the first authenticated request has no `lidc`,
        # LinkedIn 302s to set one, and it never converges ("Maximum redirects
        # followed"). The contact fetcher hits the same resolve endpoint through
        # this transport and returns 200, so it is proven, not assumed.
